...
`cargo test --locked -- --include-ignored`: all 55 tests passed with the bundled Stockfish executable and vision model. Coverage includes saved settings, Endurance ranking and exact-score parsing, a live Endurance search, checkmate and stalemate exclusion, material balancing, shallow-search fallback, book legality, legal moves across all six modes, repeated searches, overlay geometry, recovery behavior, and a vision-model load/inference smoke test.
